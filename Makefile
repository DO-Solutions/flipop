...
image: dev-image
	docker build --build-arg=FLIPOP_VERSION=$(COMMIT) -t diogoaav/flipop:latest .
ifdef latest
	docker tag diogoaav/flipop:latest diogoaav/flipop:latest
endif
...
image-push: image
	docker push diogoaav/flipop:latest
ifdef latest
	docker push diogoaav/flipop:latest
endif
